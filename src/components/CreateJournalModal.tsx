    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 modal-backdrop animate-fade">
      <div className="wireframe max-w-md w-full max-h-[90vh] overflow-y-auto animate-scale p-6 space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-black text-ink serif">写日记</h3>
          <button onClick={onClose} className="p-1.5  hover:bg-ink/5 text-ink/40"><X size={16} /></button>
        </div>

        <div>
          <label className="text-[9px] font-bold text-ink/30 uppercase tracking-widest block mb-1.5">日期</label>
          <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="wireframe-input" />
        </div>

        <div>
          <label className="text-[9px] font-bold text-ink/30 uppercase tracking-widest block mb-2">心情</label>
          <div className="flex flex-wrap gap-1.5">
            {moods.map((m) => (
              <button key={m} onClick={() => setMood(m)}
                className={`text-[10px] px-3 py-1.5  font-bold tracking-wider transition-all ${
                  mood === m ? "bg-theme text-white" : "bg-ink/5 text-ink/40 hover:bg-ink/10"
        ...
        <div>
          <label className="text-[9px] font-bold text-ink/30 uppercase tracking-widest block mb-2">能量</label>
          <div className="flex gap-2">
            {energies.map((e) => (
              <button key={e} onClick={() => setEnergy(e)}
                className={`text-[10px] px-3 py-1.5  font-bold tracking-wider transition-all ${
                  energy === e ? "bg-coral text-white" : "bg-ink/5 text-ink/40 hover:bg-ink/10"
        ...
        <div>
          <label className="text-[9px] font-bold text-ink/30 uppercase tracking-widest block mb-1.5">正文</label>
          <textarea value={content} onChange={(e) => setContent(e.target.value)}
            placeholder="发生了什么？感受到什么？" rows={4} className="wireframe-input" />
        </div>

        <div>
          <label className="text-[9px] font-bold text-ink/30 uppercase tracking-widest block mb-1.5">标签</label>
          <input value={tagsInput} onChange={(e) => setTagsInput(e.target.value)}
            placeholder="散步, 放松, 日常" className="wireframe-input" />
        </div>

        <button onClick={handleSave} disabled={!content.trim()}
          className="wireframe-btn w-full ! disabled:opacity-30 disabled:cursor-not-allowed">
          保存日记
        </button>
      </div>
    </div>
