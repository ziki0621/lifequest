      ...
      <div className="glass rounded-3xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto animate-scale p-6 space-y-4">
        ...
        <div>
          ...
          <div className="space-y-2">{stageList.map((s, i) => (
            <div key={s.id} className="flex items-center gap-2"><span className="text-[10px] font-bold text-navy/20 w-5">{i + 1}.</span><input value={s.title} onChange={(e) => updateStage(s.id, e.target.value)} placeholder={`阶段 ${i + 1}`} className="input flex-1 !py-2 !text-[11px]" /><button onClick={() => removeStage(s.id)} className="text-navy/20 hover:text-coral text-xs">✕</button></div>
          ))}</div>
