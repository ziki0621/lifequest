    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 modal-backdrop animate-fade">
      <div className="wireframe max-w-lg w-full max-h-[90vh] overflow-y-auto animate-scale p-6 space-y-5">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-black text-ink serif flex items-center gap-2"><Sparkles size={16} className="text-coral" /> AI 任务线生成器</h3>
          <button onClick={onClose} className="p-1.5  hover:bg-ink/5 text-ink/40"><X size={16} /></button>
        </div>

        <div>
          <label className="text-[9px] font-bold text-ink/30 uppercase tracking-widest block mb-1.5">输入你的生活目标 <span className="text-coral">*</span></label>
        ...
        {draft && (
          <div className="space-y-3 border-t border-[#4A3B2C]/5 pt-4">
            {draft.rationale && <p className="text-[11px] text-ink/50 leading-relaxed">{draft.rationale}</p>}

            {draft.mainQuest && (
            <div className="space-y-2">
              <h4 className="text-[10px] font-bold text-ink/30 uppercase tracking-widest flex items-center gap-1.5"><Target size={12} /> 主线任务</h4>
              <div className="bg-white/20  border border-[#4A3B2C]/5 p-3">
                <p className="text-[13px] font-black text-ink">{draft.mainQuest.title}</p>
                <p className="text-[10px] text-ink/40 mt-0.5">{draft.mainQuest.description}</p>
                <div className="mt-2 space-y-1">
                  {draft.mainQuest.stages.map((s, i) => (
                    <p key={i} className="text-[10px] text-ink/50">{i + 1}. {s.title}</p>
                  ))}
                </div>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <h4 className="text-[10px] font-bold text-ink/30 uppercase tracking-widest flex items-center gap-1.5"><ListTodo size={12} /> 日常任务</h4>
              {draft.dailyTasks.map((dt, i) => (
                <div key={i} className="bg-white/20  border border-[#4A3B2C]/5 p-2.5">
                  <p className="text-[11px] font-bold text-ink">{dt.title}</p>
                  <p className="text-[9px] text-ink/40">{dt.description}</p>
                </div>
              ))}
            </div>

            <div className="space-y-2">
              <h4 className="text-[10px] font-bold text-ink/30 uppercase tracking-widest flex items-center gap-1.5"><CheckCircle2 size={12} /> 支线任务</h4>
              {draft.sideQuests.map((sq, i) => (
                <div key={i} className="bg-white/20  border border-[#4A3B2C]/5 p-2.5">
                  <p className="text-[11px] font-bold text-ink">{sq.title}</p>
                  <p className="text-[9px] text-ink/40">{sq.description}</p>
                </div>
              ))}
            </div>
