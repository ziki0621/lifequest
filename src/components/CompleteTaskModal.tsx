  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-navy/30 backdrop-blur-sm animate-fade">
      <div className="glass rounded-3xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto animate-scale p-6 space-y-5">
        ...
        <p className="text-[13px] text-navy/60 font-medium serif leading-relaxed">
          你完成了「{task.title}」
        </p>
        ...
        <div className="flex flex-wrap gap-2">
          <span className="inline-flex items-center gap-1 text-[10px] font-bold bg-navy text-white px-3 py-1.5 rounded-full tracking-wider">
            <Sparkles size={10} /> +{task.expReward} EXP
          </span>
        ...
        <div className="border-t border-navy/5 pt-4 space-y-4">
          ...
          <textarea value={content} onChange={(e) => setContent(e.target.value)}
            placeholder="写下感受…（可跳过）" rows={3}
            className="input"
          />

          <div className="flex gap-2 justify-end">
            <button onClick={onClose} className="btn btn-ghost !text-[11px]">稍后再说</button>
            <button onClick={() => { onSaveJournal(content, mood, energy); onClose(); }} className="btn btn-primary !text-[11px]">
              保存记录
            </button>
          </div>
        </div>
