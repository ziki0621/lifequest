...
export default function QuestPlanPreview({ plan, onConfirm, onRegenerate, onClose }: Props) {
  const hasMain = plan.mainQuest && plan.mainQuest.stages?.length > 0;
  const hasDaily = plan.dailyTasks?.length > 0;
  const hasSide = plan.sideQuests?.length > 0;
