...
function AppContent() {
  ...
  return (
    <Layout currentPage={page} onNavigate={setPage}>
      ...
      {lastUndoLabel && (<div className="fixed bottom-24 md:bottom-6 left-1/2 -translate-x-1/2 z-50 animate-slide"><div className="wireframe"><div className="wireframe-inner flex items-center gap-3 px-4 py-2.5"><span className="text-[11px] font-bold text-ink">{lastUndoLabel}</span><ChamferedButton className="!h-7" onClick={undoLastMutation}><Undo2 size={11} /><span className="text-[9px] font-bold ml-1">撤销</span></ChamferedButton></div></div></div>)}
    </Layout>
  );
}
  ...
  return (
    <div className="min-h-screen flex items-center justify-center relative p-6">
      ...
      <div className="relative z-10 text-center max-w-sm animate-scale">
        <div className="w-16 h-16 mx-auto border-[1.5px] border-[#4A3B2C] bg-[#4A3B2C] text-[#F3EAD5] flex items-center justify-center mb-6 shadow-[2px_2px_0px_rgba(74,59,44,0.15)]"><Sparkles size={26} /></div>
        <h1 className="text-3xl font-black text-[#4A3B2C] tracking-tight serif mb-1">地球生活指南</h1>
        <p className="text-[10px] font-bold text-[#8B3A3A] uppercase tracking-widest">Earth Life Guide</p>
        <p className="text-[14px] text-[#4A3B2C]/60 font-medium leading-relaxed serif mt-6">把现实生活变成一场温和的 RPG。</p>
        <div className="flex items-center gap-2 my-6 text-[#4A3B2C] opacity-60">
          <div className="h-px bg-[#4A3B2C] flex-1" /><span className="text-[6px]">◇</span><div className="h-px bg-[#4A3B2C] flex-1" />
        </div>
        ...
        ].map((f) => <div key={f.text} className="flex items-center gap-3"><div className="w-8 h-8 border-[1.5px] border-[#4A3B2C] bg-[#E3D4BB] flex items-center justify-center flex-shrink-0 text-[#4A3B2C]">{f.icon}</div><span className="text-[12px] text-[#4A3B2C]/70 font-medium leading-snug">{f.text}</span></div>)}</div>
        <ChamferedButton className="!w-full !h-12 mt-10" onClick={onStart}><span className="text-[13px] font-bold tracking-widest">开始今天的生活冒险</span><ArrowRight size={16} className="ml-2" /></ChamferedButton>
        <p className="text-[10px] font-bold text-[#4A3B2C]/20 uppercase tracking-widest mt-4">没有 KPI，没有压力，只是一个温柔的陪伴。</p>
      </div>
    </div>
  );
}
