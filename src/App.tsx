        ...
        <div className="fixed bottom-24 md:bottom-6 left-4 right-4 md:left-auto md:right-6 md:w-72 z-50 space-y-2">
          {newlyUnlocked.map((a) => (
            <div key={a.id} className="glass rounded-2xl shadow-xl p-3 flex items-center gap-3 animate-slide border border-coral/20">
              <div className="w-9 h-9 rounded-full bg-coral text-white flex items-center justify-center flex-shrink-0"><Trophy size={15} /></div>
      ...
      {lastUndoLabel && (
        <div className="fixed bottom-24 md:bottom-6 left-1/2 -translate-x-1/2 z-50 animate-slide">
          <div className="glass rounded-2xl shadow-lg px-4 py-2.5 flex items-center gap-3">
            ...
            <button onClick={undoLastMutation} className="btn btn-primary !py-1 !px-3 !text-[10px] !rounded-full"><Undo2 size={12} /> 撤销</button>
          </div>
        </div>
      )}
    </Layout>
...
function WelcomeScreen({ onStart, theme }: { onStart: () => void; theme: ThemeConfig }) {
  return (
    <div className="min-h-screen flex items-center justify-center relative p-6" style={{ "--theme-primary": theme.primary, "--theme-primary-light": theme.primaryLight } as React.CSSProperties}>
      <div className="paper-bg" />
      <div className="relative z-10 text-center max-w-sm animate-scale">
        <div className="w-16 h-16 mx-auto rounded-2xl bg-theme text-white flex items-center justify-center mb-6 shadow-xl shadow-navy/20"><Sparkles size={26} /></div>
        ...
        <button onClick={onStart} className="btn btn-primary w-full mt-10 !py-3.5 !text-[13px] !tracking-widest shadow-xl shadow-navy/20">开始今天的生活冒险 <ArrowRight size={16} /></button>
...
function Feature({ icon, color, text }: { icon: React.ReactNode; color: string; text: string }) {
  return <div className="flex items-center gap-3"><div className={`w-8 h-8 rounded-full bg-white/30 flex items-center justify-center flex-shrink-0 ${color}`}>{icon}</div><span className="text-[12px] text-navy/70 font-medium leading-snug">{text}</span></div>;
}
